...
def get_posts(db:Session = Depends(get_db),current_user = Depends(oauth2.get_current_user),limit:int=10):
    ...
    query = db.query(db_models.Post).limit(limit)
...
def get_post_by_id(id:int,db:Session = Depends(get_db),current_user = Depends(oauth2.get_current_user)):
    # Retrieving posts from database using psycopg - RAW
...
def create_new_posts(payload:schemas.PostCreate,db:Session = Depends(get_db),current_user = Depends(oauth2.get_current_user)):
    ...
    posts = db_models.Post(owner_id=current_user.id,**payload.model_dump())
    db.add(posts)
    db.commit()
    db.refresh(posts) 
    # if posts is None or len(posts)==0:
...
def update_post(payload:schemas.PostUpdate,id:int,db:Session = Depends(get_db),current_user = Depends(oauth2.get_current_user)):
    # Retrieving posts from database using psycopg - RAW
